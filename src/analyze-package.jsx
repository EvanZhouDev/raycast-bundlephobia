import { getSelectedText, ActionPanel, Form, Action, Clipboard, popToRoot, Detail, showToast, Toast } from "@raycast/api";
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                let dependencies = res.dependencySizes.filter(list => {
...
|-------------------------|------------------------------|
| ${convertSecondsToFormattedTime(gzipSize / 50 / 1000)}| ${convertSecondsToFormattedTime(gzipSize / 875 / 1000)}                        |
    `
...
|------------|---------|---------------------------------------------------|`
                    ...
                    for (let depJSON of dependencies) {
                        newMarkdown += `
| \`${depJSON.name}\` | ${formatBytes(depJSON.approximateSize)} | ~${Math.round(depJSON.approximateSize / minifiedSize * 100 * 10) / 10}% |`
                    }
                }
    ...
    return (
        <Detail
            loading={markdown === "Loading data from Bundlephobia..."}
            markdown={markdown}
        />
    )
}
